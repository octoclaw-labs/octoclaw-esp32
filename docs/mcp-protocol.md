...
根据代码 (`main/protocols/protocol.cc`, `main/mcp_server.cc`)，MCP 消息是封装在基础通信协议（如 WebSocket、MQTT 或 NATS over WebSocket）的消息体中的。其内部结构遵循 [JSON-RPC 2.0](https://www.jsonrpc.org/specification) 规范。
...
1.  **连接建立与能力通告**
    ...
    - **示例 (非 MCP 负载，而是基础协议消息):**
      ```json
      {
        ...
        "transport": "websocket", // 或 "mqtt" / "nats"
        "audio_params": { ... },
