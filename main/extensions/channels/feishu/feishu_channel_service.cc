#include "feishu_channel_service.h"
...
constexpr const char* kNamespace = "octo_feishu";
...
FeishuChannelService& FeishuChannelService::GetInstance() {
    static FeishuChannelService instance;
    return instance;
}

void FeishuChannelService::Initialize(const std::string& device_id, const std::string& board_name) {
...
std::string FeishuChannelService::NormalizeSender(const std::string& raw) const {
    std::string value = ToLower(Trim(raw));
    if (value.rfind("feishu:", 0) == 0) {
        value.erase(0, strlen("feishu:"));
    }
    if (value.rfind("lark:", 0) == 0) {
        value.erase(0, strlen("lark:"));
    }
    if (value.rfind("open_id:", 0) == 0) {
        value.erase(0, strlen("open_id:"));
    }
...
void FeishuChannelService::LoadLocked() {
    ...
    cJSON_ArrayForEach(item, array) {
        ...
        if (msg.local_id.empty()) {
            msg.local_id = "feishu-" + std::to_string(msg.ts_ms);
        }
...
void FeishuChannelService::SaveConfigLocked() const {
...
void FeishuChannelService::SaveInboxLocked() const {
...
bool FeishuChannelService::UpdateConfig(bool enabled,
...
FeishuChannelService::ParseResult FeishuChannelService::ParseIncomingEventLocked(
    const cJSON* root,
    FeishuInboxMessage* message,
    std::string* error_message) const {
    if (!cJSON_IsObject(root)) {
        return ParseResult::kNotFeishuEvent;
    }
    ...
    if (channel == nullptr || ToLower(channel) != "feishu") {
        return ParseResult::kNotFeishuEvent;
    }
    ...
    const char* sender = GetStringByKeys(payload, "sender_id", "senderId", "open_id", "openId");
    const char* text = GetStringByKeys(payload, "content", "text", "message");
    if (sender == nullptr || text == nullptr) {
        if (error_message != nullptr) {
            *error_message = "feishu channel_event 缺少 sender_id 或 text";
        }
        return ParseResult::kInvalid;
    }
    ...
    const char* message_id = GetStringByKeys(payload, "message_id", "messageId", "id");
    if (message_id != nullptr) {
        message->message_id = message_id;
    }
    const char* chat_id = GetStringByKeys(payload, "chat_id", "chatId", "conversationId", "chat_id");
    if (chat_id != nullptr) {
...
bool FeishuChannelService::IsSenderAllowedLocked(const std::string& sender_id) const {
...
bool FeishuChannelService::ConsumeIncomingEvent(const cJSON* root,
                                                bool* cached,
                                                std::string* error_message) {
    if (cached != nullptr) {
        *cached = false;
    ...
    if (parsed == ParseResult::kNotFeishuEvent) {
        return false;
    }
    ...
    if (!message.message_id.empty()) {
        auto it = std::find_if(inbox_.begin(), inbox_.end(), [&message](const FeishuInboxMessage& item) {
    ...
    local_seq_ += 1;
    message.local_id = "feishu-" + std::to_string(local_seq_);
...
bool FeishuChannelService::ClearInbox() {
    std::lock_guard<std::mutex> lock(mutex_);
    inbox_.clear();
    SaveInboxLocked();
    return true;
...
cJSON* FeishuChannelService::BuildSendMessagePayloadJson(const std::string& to,
    ...
    cJSON_AddStringToObject(payload, "msgType", "text");
...
cJSON* FeishuChannelService::ExportConfigJson() const {
    std::lock_guard<std::mutex> lock(mutex_);
...
cJSON* FeishuChannelService::ExportStatsJson() const {
    std::lock_guard<std::mutex> lock(mutex_);
    cJSON* root = cJSON_CreateObject();
...
cJSON* FeishuChannelService::ExportInboxJson() const {
    std::lock_guard<std::mutex> lock(mutex_);
    cJSON* root = cJSON_CreateObject();
    ...
    for (const auto& item : inbox_) {
        ...
        cJSON_AddStringToObject(node, "chatId", item.chat_id.c_str());
